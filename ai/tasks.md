...
- [x] **T00003: Controller:** Update the `List` method in `controller/page/controller.go` to:
...
- [x] **T00005: Action URLs:** Ensure the "Edit", "Enable/Disable", and "Delete" links have correct, dynamically generated URLs (e.g., `/page/edit/{{class}}/{{identifier}}`).
- [x] **T00006: Search & Sort:** Use HTMX to make the search input and sortable table headers trigger requests to the `List` endpoint with the appropriate query parameters.
...
- [x] **T00014: Enable/Disable Functionality:**
...
- [x] **T00015: Delete Functionality:**
    - [x] Implement the full backend stack for deleting a page.
    - [x] Add a confirmation modal (using the existing `popup` function in `index.js`) to prevent accidental deletion.
